            ...
            if file_extension == "pdf":
                ...
                with io.BytesIO(file_content) as pdf_stream:
                    ...
                    for page_num, page in enumerate(doc):
                        ...
                        documents.append(Document(
                                                page_content=text,
                                                metadata={"page": page_num + 1, "source": file_name, "path": server_relative_url}
                                        ))
                    logging.info(f"PDF - Page {page_num + 1} Content: {text[:300]}...")
                return documents
                ...
                with io.BytesIO(file_content) as ppt_stream:
                    ...
                    for slide_num, slide in enumerate(presentation.slides):
                        ...
                        documents.append(Document(
                                                ...
                                                metadata={"slide": slide_num + 1, "source": file_name, "path": server_relative_url}
                                                ))
                    logging.info(f"PPTX - Slide {slide_num + 1} Content: {slide_text[:300]}...")
                return documents
                ...
                for row in sheet.iter_rows():
                    ...
                    documents.append(Document(
                                            ...
                                            metadata={"sheet": sheet.title, "source": file_name, "path": server_relative_url}
                                            ))
                    logging.info(f"XLSX - Sheet {sheet.title} Content: {sheet_text[:300]}...")
                return documents
                ...
                with io.BytesIO(file_content) as docx_stream:
                    ...
                    documents.append(Document(
                                            ...
                                            metadata={"source": file_name, "path": server_relative_url}
                                            ))
                    logging.info(f"DOCX - Content: {doc_text[:300]}...")
                return documents
            else:
